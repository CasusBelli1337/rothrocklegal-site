/**
 * Client testimonials, verbatim from the pre-redesign site (HOMEPAGE-SPEC §6).
 * Every render of these ends with `testimonialDisclaimer` (CRPC 7.1).
 */

export interface Testimonial {
  name: string;
  /** 'Client, family estate matter'. Only what the quote itself supports. */
  relationship: string;
  /** Site-relative photo, only where a real one exists. */
  photo?: string;
  quote: string;
}

...
/**
 * Body prose, then the FAQ accordion (which emits FAQPage JSON-LD), then the
 * CTA + disclaimer outro; one markdown parse feeds all three (LIBRARY-SPEC §6.2).
 */
    ...
    <div>
      ...
      {article.faqHeading && article.faq.length > 0 && (
        <section aria-labelledby={article.faqHeading.id} className="mt-14">
          ...
          <FaqAccordion items={article.faq} headingLevel="h3" className="mt-6" />
        </section>
      )}
      {article.outroHtml !== '' && (
        <div

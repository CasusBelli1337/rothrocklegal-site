...
export interface BadgeImage {
  ...
  width: number;
  height: number;
  /** A square cover (album art): rounded corners and a thin ring so a dark cover reads on the maroon band. */
  square?: boolean;
}
...
const linkTones = {
  ...
  dark: 'hover:bg-white/15 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/70',
};
...
export function Badge({ children, tone = 'brass', image, wrap, href, className = '' }: BadgeProps) {
  ...
  const chip = `inline-flex shrink-0 items-center gap-2 rounded-full px-3 py-1.5 text-[0.8125rem] font-semibold ${flow} ${tones[tone]} ${className}`;
  const content = (
    <>
      {image && (
        <Image
          ...
          width={image.width}
          height={image.height}
          className={`h-4 w-auto ${image.square ? 'rounded-sm ring-1 ring-white/40' : ''}`}
        />
      )}
      {children}
    </>
